...
for user in data_train:
    data_x_train.append([user['servicios'],user['servicios_inseguros']])
    data_y_train.append(user['peligroso'])

for user in data_predict:
    data_x_predict.append([user['servicios'],user['servicios_inseguros']])
...
tree.plot_tree(clf,filled=True,feature_names=["servicios","servicios_inseguros"],fontsize=7,class_names=["no peligroso","peligroso"])
plt.show()