...
def dashboard():
    ...
    a = plotly.utils.PlotlyJSONEncoder
    graphJSON1 = json.dumps(fig, cls=a)
    ...
    fig = go.Figure(data=[go.Table(header=dict(values=['ID',"TOTAL"]),cells= dict(values=[ids,values]))]
                    ,layout_title_text='DEVICES',layout=go.Layout(height=800))
    a = plotly.utils.PlotlyJSONEncoder
    graphJSON2 = json.dumps(fig, cls=a)
    ...
    a = plotly.utils.PlotlyJSONEncoder
    graphJSON3 = json.dumps(fig, cls=a)

    return render_template('dashboard.html',graphJSON1=graphJSON1,graphJSON2=graphJSON2,graphJSON3=graphJSON3)
